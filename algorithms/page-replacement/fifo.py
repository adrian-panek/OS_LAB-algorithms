...
def find_fault_data(data, n, frame_number) -> int:
    set = []
    # symulacja kolejki
    queue = Queue() 
    ...
    for i in range(n):
        ...
        if (len(set) < frame_number):
            # Umieszczanie danych w tablicy, jeśli jeszcze ich tam nie ma co równocześnie już zwiększa nam licznik brakujących stron
            if (data[i] not in set):
                set.append(data[i]) 
                page_fault_count += 1
                # umieszczenie danych w kolejce
                queue.put(data[i])
        # Działanie zgodnie z zasadą FIFO, jeśli kolejka jest pełna (usunięcie starej strony i umieszczenie nowej)
        else:
            ...
            if (data[i] not in set):
                ...
                set.append(data[i]) 
                # umieszczenie aktualnej strony w kolejce 
                queue.put(data[i]) 
                page_fault_count += 1
...
file = open("../../output/page-replacement/fifo-output-3.txt", "w+")
file.write(output)
file.close()
